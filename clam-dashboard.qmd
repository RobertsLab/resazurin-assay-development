...
aucFallbackActive = (aucPlotMode || cumAucPlotMode) && (
  (dataViewSel === "Measurement-normalized fluorescence" && !measurementReady) ||
  (dataViewSel === "Paper metabolism (fold-change / selected measurement)" && !paperMeasurementReady)
)

aucCalcRows = (() => {
  const sourceRows = aucFallbackActive ? filtered : plotted
  const sourceField = aucFallbackActive ? "normalized_value" : metricField
  const groups = new Map()

  for (const d of sourceRows) {
    if (!Number.isFinite(d.time_hr) || !Number.isFinite(d[sourceField])) continue
    ...
    if (!groups.has(key)) {
      groups.set(key, {
        ...
        trace_id: traceId,
        well_ids: [d.well_id],
        values: [d[sourceField]]
      })
    } else {
      const g = groups.get(key)
      g.values.push(d[sourceField])
      if (!g.well_ids.includes(d.well_id)) g.well_ids.push(d.well_id)
    }
  }
...
aucStatsContext = (() => {
  if (!aucPlotMode || !activeGroupCol || activeWithinGroupCol !== null || activeAggregationMode === "Individual replicates") {
    return {
      relevantStats: [],
      pairwiseStats: [],
      significantPairs: [],
  ...
  return {
    ...
    analysisUnitType: "sample_id / well",
    ...
    aucSignificanceNote: `Significance notation: groups sharing a letter are not significantly different after BH-adjusted pairwise Wilcoxon tests (p_adj > 0.05). Different letters indicate at least one significant pairwise difference (p_adj <= 0.05).`,
    formatP: v => {
...
timeSourceRows = cumAucPlotMode ? aucCalcRows : plottedSorted
...
aucMetricDescription = aucFallbackActive
  ? "normalized fluorescence (fallback; no measurement column selected)"
...
measurementUnavailable = dataViewSel === "Measurement-normalized fluorescence" && !aucFallbackActive && (!measurementReady || plottedSorted.length === 0)
paperUnavailable = dataViewSel === "Paper metabolism (fold-change / selected measurement)" && !aucFallbackActive && (!paperMeasurementReady || plottedSorted.length === 0)
...
groupUnavailable
  ...
  : (() => {
      ...
      if (aucPlotMode) {
        ...
        if (aucFallbackActive) {
          const fallbackLine = document.createElement("div")
          fallbackLine.textContent = "No measurement column selected for this view; using normalized fluorescence for AUC integration."
          fallbackLine.style.fontWeight = "400"
          fallbackLine.style.color = "#7c2d12"
          subtitleWrap.appendChild(fallbackLine)
        }
      }
      ...
      if (cumAucPlotMode) {
        ...
        if (aucFallbackActive) {
          const fallbackLine = document.createElement("div")
          fallbackLine.textContent = "No measurement column selected for this view; using normalized fluorescence for cumulative AUC integration."
          fallbackLine.style.fontWeight = "400"
          fallbackLine.style.color = "#7c2d12"
          subtitleWrap.appendChild(fallbackLine)
        }
      ...
      if (aucPlotMode) {
        ...
        if (aucStatsContext.relevantStats.length === 0) {
          ...
          const kwText = aucStatsContext.kwBest
            ? `Omnibus result: ${Number(aucStatsContext.kwBest.p_adj) < 0.05 ? "groups differ" : "no evidence that groups differ"} (Kruskal-Wallis p = ${aucStatsContext.formatP(aucStatsContext.kwBest.p_value)}, BH-adjusted p = ${aucStatsContext.formatP(aucStatsContext.kwBest.p_adj)}).`
            : "Omnibus result: unavailable for this selection."

          const sigPairText = aucStatsContext.significantPairs.length > 0
            ? `Pairwise differences after BH correction: ${aucStatsContext.significantPairs.map(r => `${r.group1} vs ${r.group2} (p_adj = ${aucStatsContext.formatP(r.p_adj)})`).join(", ")}.`
