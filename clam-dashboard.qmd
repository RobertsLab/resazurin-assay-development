...
resources:
  ...
  - output/clam/dashboard-stats.csv
---
...
stats_path <- file.path("output", "clam", "dashboard-stats.csv")
...
is_valid_group_value <- function(x) {
  value <- toupper(trimws(as.character(x)))
  !is.na(value) & value != "" & !value %in% c("NA", "N/A", "NULL", "NAN", "<NA>")
}

first_non_empty <- function(x) {
  x <- normalize_group_value(x)
  x <- x[!is.na(x)]
  if (length(x) == 0) {
...
trapezoid_auc <- function(time_hr, value) {
  ...
  ord <- order(time_hr, value)
  time_hr <- time_hr[ord]
  value <- value[ord]

  auc <- 0
  for (i in 2:length(time_hr)) {
    dt <- time_hr[i] - time_hr[i - 1]
    if (is.finite(dt) && dt > 0) {
      auc <- auc + dt * ((value[i] + value[i - 1]) / 2)
    }
  }
...
fit_auc_emmeans_stats <- function(data, group_col, scope_cols = c("experiment_dir", "plate_id"), value_col = "value") {
  if (!has_emmeans || !group_col %in% names(data) || !value_col %in% names(data)) {
    return(tibble())
  }
  ...
  trace_data <- data %>%
    ...
    mutate(
      trace_id = if_else(
        !is.na(sample_id_group) & normalize_group_value(sample_id_group) != "",
        paste(experiment_dir, plate_id, normalize_group_value(sample_id_group), sep = "|"),
        paste(experiment_dir, plate_id, well_id, sep = "|")
      )
    ) %>%
    group_by(experiment_dir, plate_id, trace_id) %>%
    arrange(time_hr, read_datetime, source_name, .by_group = TRUE) %>%
    summarise(
      auc_value = trapezoid_auc(time_hr, .data[[value_col]]),
      group_value = first_non_empty(.data[[group_col]]),
      n_timepoints = sum(is.finite(time_hr) & is.finite(.data[[value_col]])),
      .groups = "drop"
    ) %>%
    filter(
      is.finite(auc_value),
      is_valid_group_value(group_value)
    )
  ...
  scope_cols <- intersect(scope_cols, names(trace_data))
  if (length(scope_cols) == 0) {
    scope_groups <- list(trace_data)
  } else {
    scope_groups <- trace_data %>%
      group_by(across(all_of(scope_cols))) %>%
      group_split(.keep = TRUE)
  }

  map_dfr(scope_groups, function(scope_df) {
    scope_df <- scope_df %>%
      mutate(
        group_value = normalize_group_value(group_value)
      ) %>%
      filter(is_valid_group_value(group_value))

    group_levels <- sort(unique(scope_df$group_value))
    if (nrow(scope_df) == 0 || length(group_levels) < 2) {
      return(tibble())
    }

    scope_df <- scope_df %>%
      mutate(group_factor = factor(group_value, levels = group_levels))

    model <- stats::lm(auc_value ~ group_factor, data = scope_df)
    emm <- emmeans::emmeans(model, ~ group_factor)
    pairwise <- as.data.frame(summary(pairs(emm, adjust = "tukey")))
    omnibus <- as.data.frame(summary(emmeans::joint_tests(model)))

    scope_meta <- scope_df %>%
      summarise(
        experiment_dir = first_non_empty(experiment_dir),
        plate_id = first_non_empty(plate_id),
        n_traces = dplyr::n(),
        .groups = "drop"
      )

    omnibus_row <- omnibus %>%
      filter(`model term` == "group_factor") %>%
      slice(1) %>%
      transmute(
        comparison_type = "omnibus",
        comparison = "emmeans::joint_tests",
        group_col = group_col,
        ...
        median1 = NA_real_,
        median2 = NA_real_,
        analysis_unit_type = "trajectory / AUC",
        model_class = "emmeans / lm",
        p_adjust_method = "Tukey",
        experiment_dir = scope_meta$experiment_dir,
        plate_id = scope_meta$plate_id,
        n_traces = scope_meta$n_traces,
    ...
    pairwise_rows <- pairwise %>%
      mutate(
        ...
        n1 = map_int(group1, ~ sum(scope_df$group_value == .x, na.rm = TRUE)),
        n2 = map_int(group2, ~ sum(scope_df$group_value == .x, na.rm = TRUE)),
        median1 = map_dbl(group1, ~ as.numeric(stats::median(scope_df$auc_value[scope_df$group_value == .x], na.rm = TRUE))),
        median2 = map_dbl(group2, ~ as.numeric(stats::median(scope_df$auc_value[scope_df$group_value == .x], na.rm = TRUE)))
      ) %>%
      transmute(
        comparison_type = "pairwise",
        comparison = "emmeans::pairs",
        group_col = group_col,
        contrast,
        group1,
        group2,
    ...
    bind_rows(omnibus_row, pairwise_rows)
  })
}
...
active_measurement_cols <- intersect(
  measurement_cols,
  names(plate_data_out)[sapply(names(plate_data_out), function(col)
    any(is.finite(plate_data_out[[col]]))
  )]
)

if (length(active_measurement_cols) > 0) {
  initial_vals <- plate_data_out %>%
    filter(!is_blank, !exclude_from_analysis, is.finite(time_hr), is.finite(value)) %>%
    group_by(experiment_dir, plate_id, well_id) %>%
    slice_min(time_hr, n = 1, with_ties = FALSE) %>%
    select(experiment_dir, plate_id, well_id, value_initial = value) %>%
    ungroup()

  plate_data_out <- plate_data_out %>%
    left_join(initial_vals, by = c("experiment_dir", "plate_id", "well_id"))

  for (mc in active_measurement_cols) {
    meas_norm_col  <- paste0("meas_norm_", mc)
    paper_col      <- paste0("paper_metabolism_", mc)
    plate_data_out <- plate_data_out %>%
      mutate(
        !!meas_norm_col := if_else(
          is.finite(.data[[mc]]) & .data[[mc]] > 0 & is.finite(value) & value > 0,
          value / .data[[mc]],
          NA_real_
        ),
        !!paper_col := if_else(
          is.finite(.data[[mc]]) & .data[[mc]] > 0 &
            is.finite(value_initial) & value_initial > 0 &
            is.finite(value),
          (value / value_initial) / .data[[mc]],
          NA_real_
        )
      )
  }
...
stats_out <- if (has_emmeans) {
  group_candidates <- names(plate_data_out)[str_detect(names(plate_data_out), "_group$")]

  view_list <- list(
    list(data_view = "raw",        value_col = "value",           measurement_col = NA_character_),
    list(data_view = "normalized", value_col = "normalized_value", measurement_col = NA_character_),
    list(data_view = "delta",      value_col = "delta_value",      measurement_col = NA_character_)
  )
  for (mc in active_measurement_cols) {
    view_list <- c(view_list, list(
      list(data_view = "measurement_normalized", value_col = paste0("meas_norm_", mc),     measurement_col = mc),
      list(data_view = "paper_metabolism",       value_col = paste0("paper_metabolism_", mc), measurement_col = mc)
    ))
  }

  map_dfr(view_list, function(view) {
    if (!view$value_col %in% names(plate_data_out)) return(tibble())
    map_dfr(group_candidates, function(group_col) {
      per_plate <- fit_auc_emmeans_stats(plate_data_out, group_col,
                                         scope_cols = c("experiment_dir", "plate_id"),
                                         value_col = view$value_col)
      exp_level <- fit_auc_emmeans_stats(plate_data_out, group_col,
                                          scope_cols = c("experiment_dir"),
                                          value_col = view$value_col) %>%
        mutate(plate_id = "")
      bind_rows(per_plate, exp_level)
    }) %>%
      mutate(
        metric          = "auc",
        data_view       = view$data_view,
        measurement_col = view$measurement_col
      )
  }) %>%
    select(
      metric,
...
write_csv(stats_out, stats_path)
...
aucStatsContext = (() => {
  ...
  const dataViewToCsvView = {
    "Raw fluorescence": "raw",
    "Normalized fluorescence": "normalized",
    "Delta fluorescence": "delta",
    "Measurement-normalized fluorescence": "measurement_normalized",
    "Paper metabolism (fold-change / selected measurement)": "paper_metabolism"
  }
  const csvDataView = dataViewToCsvView[dataViewSel] || null
  const csvNeedsMeasurement = csvDataView === "measurement_normalized" || csvDataView === "paper_metabolism"
  const csvMeasurementCol = csvNeedsMeasurement ? (activeMeasurementCol || null) : null

  const allCsvStatsRows = (csvDataView === null || (csvNeedsMeasurement && csvMeasurementCol === null)) ? [] : statsArr.filter(row =>
    row != null &&
    String(row.metric || "") === "auc" &&
    String(row.data_view || "raw") === csvDataView &&
    (!csvNeedsMeasurement || String(row.measurement_col || "") === String(csvMeasurementCol)) &&
