  ...
  : (() => {
      ...
      const attachHoverEmphasis = plotNode => {
        ...
        const normalizeColor = c => {
          const value = (c || "").replace(/\s+/g, "").toLowerCase()
        ...
        const bindListeners = (attempt = 0) => {
          ...
          const pathHoverDistance = 18

          const distanceToPath = (node, x, y) => {
            const total = node.getTotalLength?.()
            const ctm = node.getScreenCTM?.()
            if (!total || !ctm) return Infinity
          ...
          const findNearbyMark = (x, y) => {
            let best = null
            let bestScore = Infinity
            marks.forEach(node => {
              const tag = node.tagName.toLowerCase()
              const rect = node.getBoundingClientRect()
              const left = rect.left - hoverPadding
              const right = rect.right + hoverPadding
              const top = rect.top - hoverPadding
              const bottom = rect.bottom + hoverPadding
              if (x < left || x > right || y < top || y > bottom) return
              let score = Infinity
              if (tag === "path") {
                score = distanceToPath(node, x, y)
                if (score > pathHoverDistance * pathHoverDistance) return
              } else {
          ...
          const onMove = ev => {
            // Check current position and nearby positions for better line detection
            const offsets = [
              {x: 0, y: 0},
              {x: 4, y: 0}, {x: -4, y: 0},
              {x: 0, y: 4}, {x: 0, y: -4},
            ...
            for (const offset of offsets) {
              ...
              hovered = stack.find(el => markSet.has(el))
              if (hovered) break
            }

            if (!hovered) {
              hovered = findNearbyMark(ev.clientX, ev.clientY)
            }
