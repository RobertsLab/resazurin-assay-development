...
fit_auc_emmeans_stats <- function(data, group_col, scope_cols = c("experiment_dir", "plate_id")) {
  if (!has_emmeans || !group_col %in% names(data)) {
    return(tibble())
  }
  ...
  trace_data <- data %>%
    ...
    summarise(
      auc_value = trapezoid_auc(time_hr, value),
      group_value = first_non_empty(.data[[group_col]]),
      n_timepoints = sum(is.finite(time_hr) & is.finite(value)),
      .groups = "drop"
    ) %>%
    filter(
      is.finite(auc_value),
...
aucStatsContext = (() => {
  ...
  // When viewing "All" plates, prefer experiment-level stats (plate_id empty/null)
  // over per-plate stats. Fall back to per-plate if no experiment-level rows exist.
  const expLevelRows = allCsvStatsRows.filter(r => !r.plate_id || String(r.plate_id).trim() === "")
  const csvStatsRows = (activePlateSel === "All" && expLevelRows.length > 0)
    ? expLevelRows
    : allCsvStatsRows.filter(r => r.plate_id && String(r.plate_id).trim() !== "")
