...
aucCalcRows = (() => {
  if (!aucFallbackActive) return plottedSorted

  // Fallback for AUC views: integrate blank-normalized fluorescence when
  // measurement-dependent views are selected but no measurement column exists.
  const groups = new Map()
  const showIndiv = activeAggregationMode === "Individual replicates" && effectiveSeriesCol !== null

  for (const d of filtered) {
    if (!Number.isFinite(d.time_hr) || !Number.isFinite(d.normalized_value)) continue
    ...
    if (!groups.has(key)) {
      groups.set(key, {
        ...
        trace_id: traceId,
        well_ids: [d.well_id],
        values: [d.normalized_value]
      })
    } else {
      const g = groups.get(key)
      g.values.push(d.normalized_value)
      if (!g.well_ids.includes(d.well_id)) g.well_ids.push(d.well_id)
    }
  }
