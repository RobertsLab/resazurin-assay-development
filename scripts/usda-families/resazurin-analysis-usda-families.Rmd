...
blanks<-full_data%>%
  ...
  summarise(mean_blank=mean(resazurin_counts));blanks
```
...
#normalize by area
full_data<-full_data%>%
  mutate(fluorescence.norm.mm2=fluorescence.norm/area)

#normalize by length=
full_data<-full_data%>%
  mutate(fluorescence.norm.mm=fluorescence.norm/length.mm)
```
...
```{r}
full_data%>%
  ggplot(aes(x=timepoint, y=fluorescence.norm.mm2, colour=temperature, group=interaction(date, well)))+
  facet_wrap(~family*date)+
  geom_point()+
  geom_line()+
  scale_colour_manual(values=c("darkblue", "darkred"))+
...
```{r}
full_data%>%
  ggplot(aes(x=timepoint, y=fluorescence.norm.mm, colour=temperature, group=interaction(date, well)))+
  facet_wrap(~family*date)+
  geom_point()+
  geom_line()+
  scale_colour_manual(values=c("darkblue", "darkred"))+
...
Remove outliers that have very high values outside the range expected. 
...
full_data<-full_data%>%
  filter(!c(date=="20250520" & sample=="B5"))%>%
  filter(!c(date=="20250520" & sample=="E5"))
```
...
full_data<-full_data%>%
  rename(value=fluorescence.norm.mm2)
```
...
Subtract blank values from abs values for oysters. 
...
full_data<-full_data%>%
  filter(!type=="blank")%>%
  mutate(abs.corr=resazurin_counts-mean_blank)
```
...
Calculate size normalized abs at each time point normalized to the starting value at time 0. 
```{r}
full_data<-full_data%>%
  group_by(date, well, family)%>%
  arrange(date, well)%>%
  mutate(abs.norm=abs.corr/first(abs.corr))
```
...
#normalize by area
full_data<-full_data%>%
  mutate(abs.norm.mm2=abs.norm/area)

#normalize by length=
full_data<-full_data%>%
  mutate(abs.norm.mm=abs.norm/length.mm)
```
...
```{r}
full_data%>%
  ggplot(aes(x=timepoint, y=abs.norm.mm2, colour=temperature, group=interaction(date, well)))+
  facet_wrap(~family*date)+
  geom_point()+
  geom_line()+
  scale_colour_manual(values=c("darkblue", "darkred"))+
...
```{r}
full_data%>%
  ggplot(aes(x=timepoint, y=abs.norm.mm, colour=temperature, group=interaction(date, well)))+
  facet_wrap(~family*date)+
  geom_point()+
  geom_line()+
  scale_colour_manual(values=c("darkblue", "darkred"))+
