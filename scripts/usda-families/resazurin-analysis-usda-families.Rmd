...
```{r}
full_data%>%
  ggplot(aes(x=timepoint, y=value.mm2, colour=temperature, group=interaction(date, well)))+
  facet_wrap(~family*date)+
  geom_point()+
  geom_line()+
  scale_colour_manual(values=c("darkblue", "darkred"))+
...
```{r}
full_data%>%
  ggplot(aes(x=timepoint, y=value.mm, colour=temperature, group=interaction(date, well)))+
  facet_wrap(~family*date)+
  geom_point()+
  geom_line()+
  scale_colour_manual(values=c("darkblue", "darkred"))+
...
```{r}
full_data%>%
  ggplot(aes(x=timepoint, y=value.mm2, colour=family, group=interaction(date, family, well)))+
  facet_wrap(~date)+
  geom_point()+
...
high_rows<-segment_data%>%
  ...
  arrange(date, sample)%>%
  mutate(value.mm2=value.mm2-first(value.mm2))%>%
  mutate(value.mm=value.mm-first(value.mm))
...
```{r}
segment_data%>%
  ggplot(aes(x=time_exposure, y=value.mm2, colour=temperature))+
...
slopes <- nested %>%
  mutate(
    model = map(data, ~ lm(value.mm2 ~ time_exposure, data = .x)),        # fit model
...
```{r}
ggplot(segment_data, aes(x = time_exposure, y = value.mm2, color=temperature)) +
  geom_point() +
...
These models do a good job representing the data. There is one sample (D3) that doesn't fit as well due to high values in the middle of the trial (non linear). The rest look like they have a good fit. In most cases slope is lower at high temperature. 
...
No oysters died on 20250506. Oysters A3, A5, B3, and B5 died on 20250513. B3 and A5 had the highest rates and those are ones that died. Oyster A4 and A5 died on 20250520 and those values are also the highest for family A. 
...
Family A has higher metabolic rates than all other families. Family A had higher rates than Family C at ambient temperature. But family A had higher rates than C, D, and E under 42C. This family may be more sensitive to stress. 
...
Calculate absorbance at each time point normalized to the starting value at time 0. 
```{r}
full_data<-full_data%>%
  group_by(date, well, family)%>%
  arrange(date, well)%>%
  mutate(absorbance.norm=resazurin_counts/first(resazurin_counts))
```
...
```{r}
full_data%>%
  ggplot(aes(x=timepoint, y=absorbance.norm, colour=temperature, group=interaction(date, well)))+
  facet_wrap(~family)+
  geom_point()+
  geom_line()+
  scale_colour_manual(values=c("darkblue", "darkred"))+
...
blanks<-full_data%>%
  filter(type=="blank")%>%
  group_by(date, timepoint)%>%
  summarise(mean_blank=mean(absorbance.norm));blanks
```
...
full_data<-full_data%>%
  filter(!type=="blank")%>%
  mutate(value=absorbance.norm-mean_blank)
```
...
ggsave(plot1a, filename="figures/usda-families/ab_family_trajectories.png", width=8, height=8, units="in")
```
