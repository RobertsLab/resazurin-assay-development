...
```{r}
plot1<-full_data%>%
  ggplot(aes(x=timepoint, y=value, colour=temperature, group=interaction(date, well), fill=temperature))+
  facet_grid(~family)+
  geom_point(alpha=0.3)+
  geom_vline(xintercept=4, linetype="dashed")+
  scale_colour_manual(values=c("darkgray", "darkred"), name="")+
  scale_fill_manual(values=c("darkgray", "darkred"), name="")+
  geom_line()+
  ylab(expression(paste("Fold Change in Fluorescence (mm" ^-1, ")")))+
  xlab("Hours")+
  ggtitle("A")+
  #geom_smooth(aes(group=interaction(family, temperature)), method="lm")+
  theme_classic();plot1

ggsave(plot1, filename="figures/usda-families/fl_family_trajectories.png", width=12, height=5, units="in")
```
...
#replicate data from time point 3 as time of exposure 0 for 40C treatment and time of exposure 3 for 18C treatment
...
high_temp_rows <- hour3_rows %>%
  mutate(temperature = "40C", time_exposure = 0)
...
```{r}
seg_plot<-segment_data%>%
  ggplot(aes(x=time_exposure, y=value, fill=temperature, colour=temperature, group=interaction(date,well, temperature)))+
  facet_grid(~family)+
  geom_point(alpha=0.3)+
  ...
  xlab("Hours")+
  ggtitle("")+
  ylab(expression(paste("Metabolism (fold change fluorescence mm" ^-1, ")")))+
  theme_classic();seg_plot

ggsave(seg_plot, filename="figures/usda-families/fl_family_averages.png", width=12, height=5, units="in")
```
...
emm<-emmeans(model, ~temperature|family)
pairs(emm)
```
...
slope_summary <- slopes %>%
  ...
  mutate(
    absolute_change = `40C` - `18C`,
    percent_change = 100 * (`40C` - `18C`) / `18C`
  )
```
...
Calculate total percent mortality for each family. 
```{r}
head(mortality)

mortality%>%
  mutate(family = str_extract(sample, "[A-Za-z]"))%>%
  group_by(family, mortality) %>%
...
## Size and mortality effects 

Calculate AUC prior to size normalization. 

```{r}
auc.size<-full_data%>%
  filter(temperature %in% c("18C"))%>%
  ungroup()%>%
  select(date, well, sample, family, timepoint, length.mm, fluorescence.corr)%>%
  
  mutate(time=as.numeric(timepoint))%>%
  group_by(date, well, sample, length.mm) %>%
  dplyr::summarise(AUC = trapz(time, fluorescence.corr));auc.size
...
```{r}
auc.size%>%
  write.csv("output/size/usda_families_size.csv")
```
...
Calculate absorbence as the reciprocal. 
```{r}
...
plot7<-corr_data%>%
  ...
  xlab(expression(paste("Scaled Fluorescence (mm" ^-1, ")")))+
  ylab(expression(paste("Scaled Absorbance (mm" ^-1, ")")))+ 
  ggtitle("A")+
  theme_classic(); plot7
...
## Run a Bland–Altman Analysis (Agreement Assessment) 

Analysis examines degree of agreement between fluorescence and absorbance. Will show if there is disagreement and to what degree absorbance over or underestimates fluorescence values. 

```{r}
# Normalize both signals
abs_norm <- (corr_data$abs.corr.mm - mean(corr_data$abs.corr.mm)) / sd(corr_data$abs.corr.mm)
fluor_norm <- (corr_data$fluorescence.corr.mm - mean(corr_data$fluorescence.corr.mm)) / sd(corr_data$fluorescence.corr.mm)

# Bland–Altman calculations
mean_values <- (abs_norm + fluor_norm) / 2
diff_values <- abs_norm - fluor_norm 

# Bland–Altman plot
plot(mean_values, diff_values,
     main = "",
     xlab = "Mean of Absorbance and Fluorescence",
     ylab = "Difference in Absorbance and Fluorescence")

abline(h = mean(diff_values), col = "black")  # mean bias
abline(h = mean(diff_values) + 1.96 * sd(diff_values), col = "black", lty = 2)
abline(h = mean(diff_values) - 1.96 * sd(diff_values), col = "black", lty = 2)

lines(lowess(mean_values, diff_values), col = "red", lwd = 2)

# Add figure panel label "B" in the upper left
mtext("B", side = 3, line = 0.5, adj = -0.2, cex = 1, font = 1)
```

We are looking for random distribution of values around the red line and within the blue lines. 

The interpretation is that: 

- Because both signals were normalized (mean = 0, SD = 1), this plot compares the relative performance of absorbance vs. fluorescence on a common scale.
- There's a nonlinear pattern: the differences vary systematically with the signal magnitude. At low mean values (left side), absorbance underestimates fluorescence (positive differences). At intermediate values (around 0–1), there's more agreement. At higher mean values (right side), absorbance overestimates or deviates erratically.
- This is strong evidence of non-constant bias — the performance of absorbance is not only inconsistent across the range of values, but it fails differently at different signal levels.
-  There’s a lot of variability in how absorbance matches fluorescence
- There is worsening agreement at higher signal levels — another mark against absorbance as a substitute.
- Absorbance does not reliably replicate fluorescence readings and exhibits nonlinear, signal-dependent bias with high variability.
