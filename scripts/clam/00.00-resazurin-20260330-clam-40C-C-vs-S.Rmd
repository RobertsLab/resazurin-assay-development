...
fig_dir   <- file.path(proj_root, "figures", "clam",
                        "20260330-clam-40C-C-vs-S")
...
# Colorblind-friendly Okabe-Ito palette (8 distinct colours)
okabe_ito <- c(
...
# Families get the first N slots; treatments get the next M slots (wrap if needed)
fam_colours <- setNames(
  okabe_ito[(seq_len(n_fam) - 1L) %% length(okabe_ito) + 1L],
  families
)
trt_colours <- setNames(
  okabe_ito[(n_fam + seq_len(n_trt) - 1L) %% length(okabe_ito) + 1L],
  treatments
)
...
# Blank wells get neutral grey so they don't collide with family colours
plate_well_colours <- c(blank = "grey40", fam_colours)
...
if (nrow(excluded_wells) > 0) {
  print(knitr::kable(excluded_wells,
                     col.names = c("Plate", "Well", "Sample",
                                   "Family", "Treatment",
                                   rep("Reason",
                                       "exclude_reason" %in%
                                         names(excluded_wells))
                                   )[seq_len(ncol(excluded_wells))]))
} else {
...
for (met in metrics_to_test) {
  ...
  grps     <- levels(df_p$x)
  fill_map <- setNames(fam_colours[str_extract(grps, "^[^:]+")], grps)
  p <- ggplot(df_p, aes(x = x, y = AUC, fill = x)) +
    geom_boxplot(alpha = 0.6, outlier.shape = NA) +
    geom_jitter(width = 0.15, alpha = 0.4, size = 1.5) +
