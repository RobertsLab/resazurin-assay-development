...
families <- sort(unique(raw_df$family_id_group))
...
p_raw_mean <- ggplot(raw_family_summary,
  ...
  geom_point(size = 2) +
  scale_colour_manual(
    values = c("blue" = "#2166ac", "tweed" = "#d6604d"),
    name = "Family") +
  scale_fill_manual(
    values = c("blue" = "#2166ac", "tweed" = "#d6604d"),
    name = "Family") +
  scale_linetype_manual(
    values = c("control" = "solid", "selected" = "dashed"),
    name = "Treatment") +
...
p_raw_by_family <- raw_df %>%
  ...
  scale_colour_manual(
    values = c("control" = "#4393c3", "selected" = "#d6604d"),
    name = "Treatment") +
  labs(x = "Time (h)", y = "Raw fluorescence (RFU)") +
  theme_classic(base_size = 12) +
  theme(strip.background = element_blank(),
...
run_ts_stats <- function(df, value_col) {
  ...
  df <- df %>%
    ...
    mutate(
      ...
      individual = factor(sample_id_group)
    )
...
individual_vars <- intersect(
  c("sample_id_group", "family_id_group", "treatment_group"),
  names(metabolism_df)
)
...
fam_colours <- c("blue" = "#2166ac", "tweed" = "#d6604d")
trt_colours <- c("control" = "#4393c3", "selected" = "#b2182b")
