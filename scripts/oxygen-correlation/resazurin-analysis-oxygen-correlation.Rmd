---
title: Oxygen correlation testing - resazurin analysis
author: "AS Huffmyer"
date: '2024'
output:
...
for (file in file_list) {
  ...
  data <- read.delim(file, header = FALSE, skip = 39)
  
  # Assign column names: first column is row labels (A–H), others are 1–12
  colnames(data) <- c("Row", as.character(1:5))
...
```{r}
metadata<-read_xlsx(path="data/oxygen-correlation/metadata.xlsx")%>%
...
Remove oysters that died and will not be measured using oxygen measurements. 

```{r}
full_data<-full_data%>%
  filter(!oyster %in% c("1-A2", "1-B8", "1-D1", "1-D9", "2-A6", "2-A8", "2-B3", "2-B6", "2-B9", "2-D1"))
```
...
ggsave(plot1, filename="figures/oxygen-correlation/individual_trajectories.png", width=8, height=8, units="in")
```
...
```{r}
ggplot(full_data, aes(x = 1, y = value)) +
  geom_violin()+
  geom_point() +
  theme_classic()
```
