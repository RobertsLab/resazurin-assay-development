...
Save files as .xlsx before loading data. 
...
Add code to cut off data once oxygen depletes past 50-60% air saturation 

Manually check weird wells: Plate 3 D5 and D6 
...
for (file in files) {
  ...
  # Subset wells (columns 53–77) and rename
  data.sub <- data[, 53:77]
...
View values for blanks vs samples. 
```{r}
plot1<-oxygen%>%
  
  ggplot(aes(x=type, y=R.mmol.min))+
  geom_boxplot()+
  geom_point()+
  theme_classic(); plot1
```

Subtract water blanks from biological samples.  

```{r}
blank_data <- subset(oxygen, c(type == "blank")) #subset to blank data only
...
Plot against size normalized oxygen. 

```{r}
plot(oxygen.bio$length.mm, oxygen.bio$R.mmol.mm.min)
```

Plot histogram of values. 

```{r}
hist(oxygen.bio$R.umol.mm.min)
```

Make a list of samples to remove with inaccurate slope extraction (based on PDF outputs where selected points do not match overall trend). 

```{r}
oxygen.bio$unique<-paste(oxygen.bio$plate, oxygen.bio$well)

remove<-c("plate3 D5", "plate3 D6", "plate 4 D5")
```
...
plot2<-oxygen.bio%>%
  ...
  ggplot(aes(x=plate, y=R.umol.mm.min))+
  geom_boxplot()+
  geom_point()+
  ylim(-0.0011,0)+
