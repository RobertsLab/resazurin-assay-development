...
```{r}
model.size<-main_data%>%
  filter(time=="4")%>%
  
  lmer(sqrt(value) ~ scale(length.mm) + (1|temperature) + (1|bag), data=.)
...
size_plot<-main_data%>%
  ...
  ggplot(data=., aes(x=length.mm, y=value, group=paste(date, bag, sample)))+
  ...
  geom_text(x=20, y=12, label="p<0.001")+
  theme_classic();size_plot

ggsave(size_plot, filename="figures/10k-seed/size_scaling.png", width=5, height=5)
```
...
model_pred1 <- glmer(next_status ~ scale(value.mm) * temperature + (1|hardening) + (1|bag:sample), family = binomial(link = "logit"), data = model_data)
