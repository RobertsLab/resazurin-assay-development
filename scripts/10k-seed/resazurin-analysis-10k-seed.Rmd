...
plot1b<-auc_data%>%
  ...
  scale_x_discrete(labels=c("18", "42"))+
...
qqPlot(residuals(model))
leveneTest(AUC ~ temperature, data=auc_data)
```
...
qqPlot(residuals(model3))
leveneTest(sqrt(value.mm) ~ time * incubation.mortality, data=high_4h_data)
```
...
plot1a<-high_4h_data%>%
  ...
  geom_point(alpha=0.3)+
  geom_smooth(aes(group=incubation.mortality), method="loess")+
...
leveneTest(AUC ~ incubation.mortality, data=auc_data_dead)
```
...
plot12<-ggplot(model_data, aes(x = value.mm, y = predicted_mortality)) +
  ...
  labs(
    title = "E",
    x = expression(paste("Metabolism (fold change fluorescence mm" ^-1, ")")),
...
## Plotting metabolism by time of mortality 

Plotting metabolism by time of mortality. 
```{r}
df_with_mortality <- main_data %>%
  group_by(date, bag, sample, hardening, incubation.mortality) %>%
  
  mutate(
    time_mortality = if (any(status == 1)) {
      time[which(status == 1)[1]]
    } else {
      NA  # if oyster never dies
    }
  ) %>%
  ungroup();df_with_mortality

#return a table with sample sizes 
df_with_mortality %>%
  distinct(date, bag, sample, incubation.mortality, time_mortality) %>%  # one row per oyster
  group_by(time_mortality, incubation.mortality)%>%
  count(time_mortality, name = "n") %>%    # count oysters per mortality time
  arrange(time_mortality)
```

Plot by each time of mortality. 

```{r}
plot_time1<-df_with_mortality%>%

  ggplot(aes(x=time, y=value.mm))+
  facet_grid(~time_mortality)+
  geom_point(alpha=0.3, colour="darkgray")+
  geom_smooth(aes(group=time_mortality), method="loess", colour="black")+
  theme_classic()+
  ylab(expression(paste("Metabolism (fold change fluorescence mm" ^-1, ")")))+
  ggtitle("")+
  xlab("Hours");plot_time1

ggsave(plot_time1, filename="figures/10k-seed/timing_mortality.png", width=6, height=5)
```
