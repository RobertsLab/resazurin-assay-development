...
```{r}
main_data%>%
  ggplot(aes(x=time, y=value, colour=scale(volume.mm3), group=paste(date, bag, sample)))+
...
ggsave(plot1, filename="figures/10k-seed/size_10k_oysters.png", width=8, height=6)
```
...
corrplot42C<-ggplot(final%>%filter(temperature=="42C"), aes(x = volume.mm3, y = value)) +
      geom_point() + 
      geom_smooth(method = "lm", se = TRUE) + 
...
ggsave(plot12, filename="figures/10k-seed/temperature_mortality-risk_alive4h_dead24h.png", width=6, height=4)
```
