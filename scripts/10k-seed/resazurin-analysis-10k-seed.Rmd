...
blanks<-main_data%>%
  ...
  summarise(mean_blank=mean(fluorescence))
```
...
main_data<-main_data%>%
  filter(!type=="blank")%>%
  mutate(fluorescence.corr=fluorescence-mean_blank)
```
...
```{r}
main_data%>%
  ggplot(aes(x=time, y=fluorescence.corr, colour=temperature, group=sample))+
...
main_data<-main_data%>%
  ...
  mutate(fluorescence.norm=fluorescence.corr/first(fluorescence.corr))
```
...
main_data$value.mm3<-main_data$fluorescence.norm/main_data$volume.mm3
```
...
```{r}
hist(main_data$fluorescence.norm)
```
...
```{r}
model.size<-lmer(fluorescence.norm ~ time * temperature * scale(volume.mm3) + (1|bag) + (1|date:bag:sample), data=main_data)
...
```{r}
main_data%>%
  ggplot(aes(x=time, y=fluorescence.norm, colour=scale(volume.mm3), group=paste(date, bag, sample)))+
...
```{r}
model.size2<-lmer(fluorescence.norm ~ temperature * scale(volume.mm3) + (1|bag), data=main_data)
...
final%>%
  ...
  ggplot(aes(x=volume.mm3, y=fluorescence.norm))+
  #facet_wrap(~temperature)+
  geom_point()+
...
18°C
```{r}
correlation18C <- lm(fluorescence.norm ~ volume.mm3, data = final%>%filter(temperature=="18C"))

corrplot18C<-ggplot(final%>%filter(temperature=="18C"), aes(x = volume.mm3, y = sqrt(value.mm3))) +
      geom_point() + 
      geom_smooth(method = "lm", se = TRUE) + 
      theme_classic();corrplot18C
  
      
correlation42C <- lm(fluorescence.norm ~ volume.mm3, data = final%>%filter(temperature=="42C"))

corrplot42C<-ggplot(final%>%filter(temperature=="42C"), aes(x = volume.mm3, y = value.mm3)) +
      geom_point() + 
      geom_smooth(method = "lm", se = TRUE) + 
      theme_classic();corrplot42C
...
alive_4h_data<-main_data%>%
  ...
  ungroup()%>%
  dplyr::select(!c(fluorescence.norm, volume.mm3, width.mm, length.mm))
...
```{r}
model<-lmer(value.mm3 ~ time * temperature * hardening + (1|bag) + (1|date:bag:sample), data=alive_4h_data)
...
Significant effects of time x temperature. 
...
ggsave(plot1, filename="figures/10k-seed/hardening_temperature_alive4h_oysters.png", width=8, height=6)
...
ggsave(plot1a, filename="figures/10k-seed/hardening_temperature_alive4h_oysters-2.png", width=8, height=6)
```
...
Build a model including all predictors. 
```{r}
model<-lmer(value.mm3 ~ time * hardening + (1|bag) + (1|date:bag:sample), data=dead_4h_data)
...
```{r}
model2<-lmer(value.mm3 ~ time * hardening + (1|bag) + (1|date:bag:sample), data=dead_4h_data)
...
ggsave(plot1a, filename="figures/10k-seed/hardening_42C_dead4h.png", width=6, height=4)
```
...
Build a model including all predictors. 
```{r}
model3<-lmer(value.mm3 ~ time * hardening * incubation.mortality + (1|bag) + (1|date:bag:sample), data=high_4h_data)
...
Significant effects of time x mortality in oysters during the 4 h incubation. 
...
ggsave(plot1a, filename="figures/10k-seed/hardening_42C_mortality_dead4h.png", width=8, height=4)
```
...
ggsave(plot12, filename="figures/10k-seed/hardening_mortality-risk_42C_dead4h.png", width=8, height=4)
```
...
Build a model including all predictors. 
```{r}
model5<-lmer(value.mm3 ~ time * temperature * hardening + (1|bag) + (1|date:bag:sample), data=alive_24h_data)
...
ggsave(plot1, filename="figures/10k-seed/hardening_temperature_alive24h.png", width=8, height=4)
...
ggsave(plot1a, filename="figures/10k-seed/hardening_temperature_alive24h-2.png", width=8, height=4)
```
...
Build a model including all predictors. 
```{r}
model6<-lmer(value.mm3 ~ time * temperature * hardening + (1|bag) + (1|date:bag:sample), data=dead_24h_data)
...
ggsave(plot1, filename="figures/10k-seed/hardening_temperature_alive4h-dead24h.png", width=8, height=4)
...
ggsave(plot1a, filename="figures/10k-seed/hardening_temperature_alive4h-dead24h-1.png", width=8, height=4)
```
...
ggsave(plot12, filename="figures/10k-seed/hardening_mortality-risk_dead24h.png", width=10, height=5)
...
Build a model including all predictors. 
```{r}
model<-lmer(value.mm3 ~ time * temperature + (1|hardening) + (1|date:bag:sample), data=alive_4h_data)
...
Build a model including all predictors. 
```{r}
model<-lmer(value.mm3 ~ time * temperature + (1|hardening) + (1|date:bag:sample), data=dead_4h_data)
...
```{r}
model2<-lmer(value.mm3 ~ time * temperature + (1|hardening) + (1|date:bag:sample), data=dead_4h_data)
...
ggsave(filename="figures/10k-seed/tempreature_dead4h.png", plot1a, width=5, height=4)
```
...
Build a model including all predictors. 
```{r}
model3<-lmer(value.mm3 ~ time * incubation.mortality + (1|hardening) + (1|date:bag:sample), data=high_4h_data)
...
plot1a<-high_4h_data%>%
  ...
  theme_classic()+
  xlab("Hour")+
  ylim(-0.01, 0.02);plot1a

ggsave(plot1a, filename="figures/10k-seed/temperature_mortality_4h.png", width=5, height=4)
```
...
Build a model including all predictors. 
```{r}
model5<-lmer(value.mm3 ~ time * temperature + (1|hardening) + (1|date:bag:sample), data=alive_24h_data)
...
```{r}
model5<-lmer(value.mm3 ~ time * temperature + (1|bag) + (1|date:bag:sample), data=alive_24h_data)
...
plot1<-alive_24h_data%>%
  ...
  theme_classic()+
  xlab("Hour")+
  ylim(-0.01, 0.02);plot1

ggsave(plot1, filename="figures/10k-seed/temperature_alive24h.png", width=5, height=4)
```
...
Build a model including all predictors. 
```{r}
model6<-lmer(value.mm3 ~ time * temperature + (1|hardening) + (1|date:bag:sample), data=dead_24h_data)
...
ggsave(plot1, filename="figures/10k-seed/temperature_alive4h_dead24h.png", width=5, height=4)
```
...
ggsave(plot12, filename="figures/10k-seed/temperature_mortality-risk_alive4h_dead24h.png", width=5, height=4)
```
...
mortality_data<-main_data%>%
  filter(temperature=="42C")%>%
  ungroup()%>%
  dplyr::select(!c(width.mm, length.mm, fluorescence.norm, volume.mm3))%>%
...
Build a model including all predictors. 
```{r}
model15<-lmer(value.mm3 ~ time * mortality + (1|hardening) + (1|date:bag:sample), data=mortality_data)
...
ggsave(plot1, filename="figures/10k-seed/mortality_metabolic-rate_comparison_42C.png", width=5, height=4)
```
...
ggsave(plot3, filename="figures/10k-seed/survival_temperature.png", width=5, height=4)
```
...
ggsave(plot3a, filename="figures/10k-seed/survival_hardening_temperature.png", width=8, height=4)
```
...
ggsave(plot3b, filename="figures/10k-seed/survival_hardening_temperature2.png", width=8, height=4)
```
