...
```{r}
model<-lmer(sqrt(value.mm) ~ timepoint * temperature * spat_treatment + (1|date:plate:well) + (1|holding_tank) + (1|spat_treatment:tank), data=full_data)
...
ggsave(plot1c, filename="figures/spat-stress/metabolism_treatment.png", width=7, height=4)
...
ggsave(plot1d, filename="figures/spat-stress/metabolism_temperature_treatment.png", width=8, height=5)
```
...
```{r}
mortality_model<-lmer((value.mm)^(1/3) ~ timepoint * mortality * spat_treatment + (1|holding_tank) + (1|spat_treatment:tank), data=data_42C)
...
ggsave(plot5a, filename="figures/spat-stress/tpc.png", width=6, height=5)
```

Analyze effect of temperature. 

```{r}
tpc_model<-lmer((value.mm)^(1/3) ~ temperature * spat_treatment + (1|holding_tank) + (1|spat_treatment:tank), data=end_data)

summary(tpc_model)
Anova(tpc_model)
```

Temperature and treatment are significant 
