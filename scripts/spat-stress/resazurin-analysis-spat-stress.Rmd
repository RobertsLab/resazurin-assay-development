...
leveneTest((value.mm)^(1/3) ~ timepoint * temperature, data=full_data)
```
...
qqPlot(residuals(model))
leveneTest((value.mm)^(1/3) ~ timepoint * temperature, data=full_data%>%filter(mortality==0))
```
...
leveneTest((value.mm)^(1/3) ~ timepoint * mortality, data=data_42C)
```
...
leveneTest((AUC)^(1/3) ~ mortality, data=auc_data)
```
