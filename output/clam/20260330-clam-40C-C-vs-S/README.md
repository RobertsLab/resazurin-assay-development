`output/clam/20260330-clam-40C-C-vs-S`
...
`scripts/clam/00.00-resazurin-20260330-clam-40C-C-vs-S.Rmd`.
All tabular files are comma-separated values (CSV).

See `data/clam/20260330-clam-40C-C-vs-S/README.md` for experimental context.
...
| `metric` | Measurement metric (e.g. `metabolism_per_area_mm2_measurement`) |
...
| `exclude_reason` | Reason for exclusion (if applicable) |
...
#### `corrected_fold_change.csv` *(legacy)*

Produced by an earlier version of the analysis pipeline. Contains the full
per-well per-timepoint data frame with blank-corrected fold-change values but
without size-normalised metabolism columns. Superseded by `metabolism.csv`.

---

#### `raw_fluorescence.csv` *(legacy)*

Produced by an earlier version of the analysis pipeline. Contains the raw
per-well per-timepoint fluorescence data merged with layout metadata.
Superseded by `metabolism.csv`.
